...
// ========== Fonctions localStorage sécurisées ==========
...
function addVideo() {
  const title = document.getElementById('video-title')?.value?.trim() || 'Vidéo sans titre';
...
function loadAdminData() {
  ...
  if (pubsContainer) {
    ...
    pubsContainer.innerHTML = pubs.map(pub => `
      <div class="admin-item">
        <img src="${pub.image}" alt="${pub.title}">
        <div class="info"><strong>${pub.title}</strong></div>
  ...
  if (productsContainer) {
    ...
    productsContainer.innerHTML = products.map(prod => `
      <div class="admin-item">
        <img src="${prod.image}" alt="${prod.name}">
        <div class="info"><strong>${prod.name}</strong> - $${prod.price} - ${prod.category}</div>
  ...
  if (lookbookContainer) {
    ...
    lookbookContainer.innerHTML = images.map(img => `
      <div class="admin-item">
        ...
        <div class="info"><strong>Image ${img.id}</strong></div>
...
function addPub() {
  ...
  reader.onload = function(e) {
    const pubs = getPubsFromStorage();
    pubs.push({ id: Date.now(), title, image: e.target.result, link: 'boutik.html' });
...
function addProduct() {
  ...
  reader.onload = function(e) {
    const products = getProductsFromStorage();
    products.push({ id: Date.now(), name, price, category, image: e.target.result });
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Si page d'accueil avec splash
  if (document.getElementById('splash')) {
    showSplash();
  }
  // Si page d'accueil : chargement des sections
  ...
  if (document.getElementById('edit-videos')) {
    loadEditVideos();
  }
  ...
  // Si page admin et déjà connecté
